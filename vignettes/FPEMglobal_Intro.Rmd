...
Each run is given a default _run name_ which is used in the names of directories to hold outputs and the output files themselves. It is composed of the date the function was called (via `format(Sys.time(), "%Y%m%d")`), the marital subpopulation of the run ("married", "unmarried", or "all_women"), the age group ("15-49"). This can be overridden by supplying a value to the `run_name` argument. The output of each function is a list with elements `run_name` and `output_dir_path`, where the latter is the path to the directory containing the results. Results are stored in a subdirectory of `output_dir_path` with name `run_name`. 
...
To combine unmarried and married women runs to create all women results, use the funtion `combine_runs`. This produces MCMC trajectories for all women from the married and unmarried women outputs specified in the arguments `married_women_run_run_dir_path` and `unmarried_women_run_run_dir_path`.
...
all_women_run_name <- combine_runs(
    ## Inputs
    married_women_run_name = married_run_name,
    unmarried_women_run_name = unmarried_run_name
    )
 ...
 make_results(run_name = all_women_run_name[["run_name"]])
...
```{r uw-base-level, echo = TRUE, results = "markup", eval = FALSE}
 ...
 post_process_mcmc(run_name = unmarried_run_name[["run_name"]])

 ## Produce plots, tables
 make_results(run_name = unmarried_run_name[["run_name"]])
...
all_women_adj_med <-
    combine_runs(married_women_run_run_dir_path = file.path("output",
                     all_women_runs$married_run_name),  
                 unmarried_women_run_run_dir_path = file.path("output",
