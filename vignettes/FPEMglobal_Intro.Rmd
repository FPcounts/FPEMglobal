...
### Adding Aggregates to an Existing Run

If a run has completed and special aggregates are desired, you must pass `special_aggregates_name` to `post_process_mcmc` and post process the output directory again, and then re-run `make_results`. If you want to add aggregates for age ratios to an all women run you will also need to run `combine_runs`. Pass `special_aggregates_name` to `make_results` and, if applicable, `combine_runs` as well. Aggregates are based on the files stored in the 'countrytrajectories' directory inside the run directory. If you are adding aggregates to an all women run and you have deleted this directory you _will_ need to delete the entire all women directory and recreate it by running `combine_runs`. Use the argument `run_name_override` to specify the name of the all women output directory. 
