...
FPEMglobal (*F*amily *P*lanning *E*stimation *M*odel global) implements a Bayesian hierarchical model for estimating and projecting the prevalence of contraceptive use and unmet need at the national level for all women of reproductive age (i.e., ages 15--49 years). There are submodels which produce estimates and projections for married and unmarried women. Together, these are used to produce results for all women. The married women model is described in @alkema_national_2013. Updates to the model are described in @cahill_modern_2017; these are included in _FPEMglobal_. The unmarried women model, and the process of producing estimates for all women regardless of marital status, is described in @kantorova18_contr. _FPEMglobal_ is a direct descendant of _ContraceptiveUse_ [@AlkemaContraceptiveUse2014].
...
2. Run the model for unmarried women
    1. Generate MCMC sample
    2. Transform and summarize
3. Combine the married and unmarried women runs to get results for all women regardless of marital status.
...
|       | `combine_runs`            | Combine married and unmarried women runs, as produced by `do_global_run`, to get all women MCMC output.     |
| 0     | `do_global_mcmc`          | Run the  model for either married or unmarried women (generates MCMC output only).                          |
|       | `add_global_mcmc`         | Run an additional MCMC chain; must run `do_global_mcmc` to get at least one chain first.                    |
|       | `post_process_mcmc`       | Post-process MCMC output produced by `do_global_mcmc` (and `add_global_mcmc`).                              |
|       | `make_results`            | Produce summary results from post-processed MCMC chains as produced by `post_process_mcmc`.                 |
...
More than one MCMC chain is required per run. This is controlled by the argument `chain_nums` which is passed to the `do_...` functions. This must be a sequence, not a single number. For example, if two chains are wanted, write `chain_nums = 1:2`; if four are wanted, write `chain_nums = 1:4`^[The _R_ syntax '`1:4`' is short-hand for `1 2 3 4`.]:
...
| Argument                                | Default                             |
|-----------------------------------------|-------------------------------------|
| `data_csv_filename`                     | data_cp_model_all_women_1549.csv    |
...
The model is estimated using an MCMC algorithm. Defaults for the number of iterations, proportion to discard as burn-in, and how to thin the sample, are set in all functions that implement the MCMC. A full run takes several hours, typically 40--48 in our experience with a 30-core computer as at 2019. 
...
```{r autopilot-fn-doc, echo=FALSE,results="hide"} 
...
Custom country aggregates can be requested if the ones produced by default are not adequate. Use the argument `special_aggregates_name`. This must be a name for the aggregate that can be used in the filenames of the output plots and tables. There must be a corresponding '.csv' file with the _same name_ as the aggregates describing the grouping of countries. It must have two columns: 'iso.country', and 'groupname'.  `special_aggregates_name` can be passed to `do_global_all_women_run` or `do_global_run` if a new run is being done. If a run has completed and special aggregates are desired, you must pass `special_aggregates_name` to `post_process_mcmc` and run it again on the output directory first, then run `make_results` (pass `special_aggregates_name` to `make_results` as well).

The special aggregates '.csv' file must be in the same directory as all other input files and you must declare this directory using the argument `input_data_folder_path`. You can specify more than one '.csv' it there are multiple aggregation schemes you need results for.

The first seven rows of a table that should be supplied to obtain special country aggregates are shown below. The first column ('A') lists country ISO codes, the second ('B') lists the group they belong to. The file should contain each ISO code only once and should be saved in '.csv' format.
...
The code below illustrates how special aggregates for the World Bank (WB) 2020 (fiscal year) income groups can be requested. The package comes with the file 'WB_Income_2020-w_middle.csv' (access this with `system.file("extdata", "WB_Income_2020-w_middle.csv", package = "FPEMglobal")`) which defines WB country aggregates. The name of the aggregate to be passed to the functions is "WB_Income_2020-w_middle".
...
```{r other-aggregates-eg, echo = TRUE, results = "markup", eval = FALSE}
 ...
 all_women_runs <-
     do_global_all_women_run(## Describe the run
         ...
         special_aggregates_name = "WB_Income_2020-w_middle" #<<< no ".csv"; just the name
     )
 
``` 

To add special aggregates to a completed run see the example at the end of the next section. You can omit the adjusted medians in that example if you do not want them. 
...
Median adjustment is done at the stage outputs are summarized as plots and tables. They are controlled by the argument `adjust_medians` (default value is `FALSE`). You can pass `adjust_medians = TRUE` to `do_global_all_women_run`, or `do_global_run` if you are doing a new run. 
...
## Adjusted medians maried and unmarried.
...
post_process_mcmc(all_women_runs$married_run_name,
             ...
             special_aggregates_name = "WB_Income_2020-w_middle")
