...
polarHistogram<-function(# Construct a polar histogram
  ### Written by
  ### C. Ladroue, after Kettunen, J. et al. Genome-wide association study identifies multiple loci influencing human serum metabolite levels. Nat Genet advance online publication (2012). URL http://dx.doi.org/10.1038/ng.1073.
  df,
  binSize=1,
  ...
  ##author<< C. Ladroue, after Kettunen and not

  # ordering
  df<-dplyr::arrange(df,family,item,score)

  # summing up to one
  # TO DO: replace NA by 0 because cumsum doesn't ignore NA's.
  df<-plyr::ddply(df,plyr::.(family,item),transform,value=cumsum(value/(sum(value))))

  # getting previous value
  df<-plyr::ddply(df,plyr::.(family,item),transform,previous=c(0,head(value,length(value)-1)))

  # family and item indices. There must be a better way to do this
  df2<-plyr::ddply(df,plyr::.(family,item),plyr::summarise,indexItem=1)
  df2$indexItem<-cumsum(df2$indexItem)
  df3<-plyr::ddply(df,plyr::.(family),plyr::summarise,indexFamily=1)
  ...
  # histograms
  p<-ggplot2::ggplot(df)+ggplot2::geom_rect(
    ggplot2::aes(
      xmin=xmin,
      xmax=xmax,
      ymin=ymin,
  ...
  dfItemLabels<-plyr::ddply(df,plyr::.(item),plyr::summarize,xmin=xmin[1])
...
#     familyLabelsDF<-ddply(df,.(family),summarise,x=mean(xmin+binSize),angle=mean(xmin+binSize)*(-360/totalLength)-alphaStart*180/pi)
    familyLabelsDF<-stats::aggregate(xmin~family,data=df,FUN=function(s) mean(s+binSize))
...
#     )
#
  # change JR, 20131105: opts deprecated in new version of ggplot2
  p<-p+ggplot2::theme(
  ...
  # nice colour scale
  p<-p+ggplot2::scale_fill_brewer(palette='Set1',type='qual')
