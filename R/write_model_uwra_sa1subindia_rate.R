...
WriteModel_InclNoData_SA1SubIndia_Rate <- function(mcmc.meta){
...
###--------------------------------------------------------------
    ...
    if(global.run || global.validation.run) {
        ...
        c.unmet ~ dunif(-35,0)                #[MCW-2016-06-02-1] Update for UWRA.
    ...
    if(incl.no.data.countries || at.random.no.data || leave.iso.out) {
     ...
     cat("
        ...
        for(c in 1:C) {
          ...
          logitomega.c[c] ~
              dnorm(w.subreg[reg.in.sex.ac.unm.SA1sub.c[c]], tau.wc)
                                # >>>>> SA MODEL <<<<<
          Romega.c[c] <-
              (0.5*exp(logitRomega.c[c])+0.01)/(1+exp(logitRomega.c[c]))
          logitRomega.c[c] ~
              dnorm(Rw.subreg[reg.in.sex.ac.unm.SA1sub.c[c]], tau.Rwc)
                                # >>>>> SA MODEL <<<<<

          ## Midyear ratio modern/total: 3-level
          RT.c[c]  ~
              dnorm(RT.subreg[reg.in.sex.ac.unm.SA1sub.c[c]], tau.RTc)#T(1800,)
                              # >>>>> SA MODEL >>>>> RATE MODEL
                              # Truncation removed as per NC's rate model.

          ## unmet need intercept
          unmet.intercept.c[c] ~
              dnorm(unmet.subreg[reg.in.sex.ac.unm.SA1sub.c[c]], tau.unmetc)
                              # >>>>> SA MODEL <<<<<
    ...
    if(global.run || global.validation.run) {
        ...
        ## In sexual activity models like this one, the 'subreg' level consists of
        ## EITHER
        ## 1. modified major areas (i.e,. 'regions') in sexual activity groups.
        ##    The correct number of these clusters is 'n.reg.in.sex.ac.unm'.
        ##    They are named in 'name.reg.in.sex.ac.unm'. The major areas are modified
        ##    to allow for, e.g,. 'Africa (SA high)' and 'Africa (SA low)'.
        ## OR
        ## 2. Modified major areas (i.e,. 'regions') nested in Sexual Activity Group 0 and
        ##    (unmodified) subregions nested within SA Group 1.
        ##    The correct number of these clusters is 'n.reg.in.sex.ac.unm.SA1sub'.
        ##    They are named in 'name.reg.in.sex.ac.unm.SA1sub'.
        ## Note the 'reg' in the names of the counters (as opposed to 'subreg').
        ## This model, 'WriteModel_InclNoData_SA1SubIndia_Rate()', is of the second kind.

        for (x in 1:n.reg.in.sex.ac.unm.SA1sub){
            unmet.subreg[x] ~ dnorm(0, tau.unmetworld)
            S.subreg[x] ~ dnorm(S.reg[sex.ac.unm.SA1sub.reg[x]] , tau.Ssubreg) #Change NC, 20160811
            w.subreg[x] ~ dnorm(w.reg[sex.ac.unm.SA1sub.reg[x]] , tau.wsubreg)#T(-4.5, 0)
            ## T.subreg[x] ~ dnorm(T.reg[sex.ac.unm.SA1sub.reg[x]] , tau.Tsubreg)#T(1800,)
                                # >> RATE MODEL << Not in rate model.
            Rw.subreg[x] ~ dnorm(Rw.reg[sex.ac.unm.SA1sub.reg[x]] , tau.Rwsubreg)#T(-4.5, 0)
            RT.subreg[x] ~ dnorm(RT.reg[sex.ac.unm.SA1sub.reg[x]] , tau.RTsubreg)#T(1800,)
        }
        ...
        ## 'Reg' level is sexual activity among unmarried
        ## category. This is coded as 1 (LOW activity) and 2 (HIGH)
        ## activity.
        for(x in 1:n.sex.ac.unm) {
            w.reg[x] ~ dnorm(w.world, tau.wreg)#T(-4.5, 0)
            Rw.reg[x] ~ dnorm(Rw.world, tau.Rwreg)#T(-4.5, 0)
            RT.reg[x] ~ dnorm(RT.world, tau.RTreg)#T(1800,)
            ## RATE MODEL >>>>>
            S.reg[x] ~ dnorm(S.world, tau.Sreg) #Change NC, 20160811
            ## <<<<< RATE MODEL
        }
    ...
    if(with(mcmc.meta$winbugs.data, isTRUE((n.not.sex.ac.unm.index > 0 && n.sex.ac.unm.index > 0)))) {
        cat("
        ## At least one country in each sexual activity group

        ## >>>>> RATE MODEL >>>>> SA MODEL

        ## set level: countries with LOW sexual activity among
        ## unmarried women
        for (i in 1:n.not.sex.ac.unm.index){
            setlevel.c[not.sex.ac.unm.index[i]] ~ dnorm(Shigher, tau.higherSc)#T(?,)
        }
        ## set level: countries with HIGH sexual activity among
        ## unmarried women, 3-level USE SEX AC UNM cluster
        for (i in 1:n.sex.ac.unm.index){
            setlevel.c[sex.ac.unm.index[i]] ~
                dnorm(S.subreg[reg.in.sex.ac.unm.SA1sub.c[sex.ac.unm.index[i]]],
                      tau.Sc)#T(?,)
        }
        ## T.reg[1] <- TOneLevel   #just an alias #not in rate model
        ## T.reg[2] <- T.world                    #not in rate model

        ", file = file.path(mcmc.meta$general$output.dir, filename.model), fill = TRUE, append = T)
    } else if(with(mcmc.meta$winbugs.data, isTRUE(n.not.sex.ac.unm.index > 0))) {
        cat("
        ## No countries in sexual activity group 1
        ...
        ## T.reg <- TOneLevel                #just an alias #not in rate model

        ", file = file.path(mcmc.meta$general$output.dir, filename.model), fill = TRUE, append = T)
    } else if(with(mcmc.meta$winbugs.data, isTRUE(n.sex.ac.unm.index > 0 ))) {
        cat("
        ## No countries in sexual activity group 0

        ## set level: countries with HIGH sexual activity among
        ## unmarried women, 3-level USE SEX AC UNM cluster
        for (i in 1:n.sex.ac.unm.index){
            setlevel.c[sex.ac.unm.index[i]] ~
                dnorm(S.subreg[reg.in.sex.ac.unm.SA1sub.c[sex.ac.unm.index[i]]], tau.Sc)#T(?,)
        }
        ## T.reg <- T.world #not in rate model
    ...
    if(incl.no.data.countries || at.random.no.data || leave.iso.out) {
      if(with(mcmc.meta$winbugs.data, isTRUE(n.not.sex.ac.unm.index > 0 && n.sex.ac.unm.index > 0))) {
        ...
        ## set level: countries with LOW sexual activity among
        ## unmarried women
        for (i in 1:n.not.sex.ac.unm.index.no.data){
            setlevel.c[not.sex.ac.unm.index.no.data[i]] ~ dnorm(Shigher, tau.higherSc)#T(?,)
        }
        ## set level: countries with HIGH sexual activity among
        ## unmarried women, 3-level USE SEX AC UNM cluster
        for (i in 1:n.sex.ac.unm.index.no.data){
            setlevel.c[sex.ac.unm.index.no.data[i]] ~
                dnorm(S.subreg[reg.in.sex.ac.unm.SA1sub.c[sex.ac.unm.index.no.data[i]]],
                      tau.Sc)#T(?,)
        }
        ## T.reg[1] <- TOneLevel   #just an alias #not in rate model
        ## T.reg[2] <- T.world                    #not in rate model
        ...
        ", file = file.path(mcmc.meta$general$output.dir, filename.model), fill = TRUE, append = T)
    } else if(with(mcmc.meta$winbugs.data, isTRUE(n.not.sex.ac.unm.index.no.data > 0))) {
        cat("
        ## No countries in sexual activity group 1
        ...
        ", file = file.path(mcmc.meta$general$output.dir, filename.model), fill = TRUE, append = T)
    } else if(with(mcmc.meta$winbugs.data, isTRUE(n.sex.ac.unm.index.no.data > 0 ))) {
        cat("
        ## No countries in sexual activity group 0
        ...
        for (i in 1:n.sex.ac.unm.index.no.data){
            setlevel.c[sex.ac.unm.index.no.data[i]] ~
                dnorm(S.subreg[reg.in.sex.ac.unm.SA1sub.c[sex.ac.unm.index.no.data[i]]], tau.Sc)#T(?,)
        }
        ## T.reg <- T.world #not in rate model
        ...
        for (z in 1:n.countriesmorethan1obs){
            for (i in 2:N.unique.c[getc.z[z]]){
                ...
                taueps.ci[getc.z[z],i] <- tau.tot.st/
                    (1-pow(rho.tot,
                           2*(gett.ci[getc.z[z],i] - gett.ci[getc.z[z],i-1])))
                           # (2019-05-13) I think this parameter is unused; could be deleted
    ...
    ", file = file.path(mcmc.meta$general$output.dir, filename.model), fill = TRUE, append = TRUE)
    if(!mcmc.meta$general$EA.bias.negative) {
        cat("
        ## EA bias negative
        for (m in 1:2){
            for (i in 2:ncat.emal){
    ...
    if(!mcmc.meta$general$HW.bias.negative) {
        cat("
