...
#  plot.tiff  = FALSE##<< TIFF-format? If FALSE, PDF-format is used.
  ...
  for (year in years){
    ...
    df<-reshape::melt(df.all,c("family","item"),variable_name="score") # from wide to long
 ...
 ,years = c(1990, 2000, 2010)
  ...
  if (is.null(output.dir)){
    ...
    if(mcmc.meta$general$do.country.specific.run) {
        warning("Once country run: polar histograms not produced.")
        return(invisible())
    }
  ...
  ## [MCW-2016-08-26-14] Include countries with no data
  if(!do.country.specific.run & mcmc.meta$general$include.c.no.data) {
      country.info <- rbind(mcmc.meta$data.raw$country.info, mcmc.meta$data.raw$country.info.no.data)
      ...
      for (year in years){
        ...
        if (res==2){
          for (reg in region.info$name.reg){
            ...
            df<-reshape::melt(df,c("family","item"),variable_name="score") # from wide to long
            p<-polarHistogram(df,familyLabels=FALSE, direction="outwards",
                              guides=c(20,40,60, 80))
            #,circleProportion = 0.9)
            pdf(file.path(fig.dir, makeFileName(paste0(run.name, "polarhist", reg, year, ".pdf"))), width = 10, height = 8) # change JR, 20140418
            print(p)
            dev.off()
          }
        } else {
